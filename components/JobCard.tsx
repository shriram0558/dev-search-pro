...
export default function JobCard({
  ...
  return (
    <li className="px-4 py-5 flex flex-col bg-secondary-dark rounded-xl text-primary gap-5 justify-between">
      <div className="flex justify-between">
        <Image
          src={`${companyIconMap.get(company)}`}
          width={24}
          height={24}
          alt="company-icon"
        />
        <div className="text-[0.6rem] flex gap-1 items-center">
          ...
          <p className="text-tertiary-dark font-bold">8 hours ago</p>
        </div>
      </div>
      <div>
        <div className="font-semibold text-lg">{jobTitle}</div>
        <p className="text-tertiary-dark text-sm font-medium">
          {company}, {location}
        </p>
      </div>
      <div className="flex gap-3">
        <p className="text-xs py-1 px-2 rounded-2xl bg-[#413f37] text-[#ddccb3] font-semibold">
          {jobMode}
        </p>
        <p className="text-xs py-1 px-2 rounded-2xl bg-[#493b43] text-[#cea9c4] font-semibold">
          {jobType === "fulltime" ? "Full Time" : "Intern"}
        </p>
      </div>
      ...
      <p className="text-xs text-tertiary-dark font-medium">{description}</p>
